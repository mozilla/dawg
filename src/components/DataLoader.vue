...
onBeforeMount(() => {
  ...
  Promise.all(
    props.sources &&
      props.sources.map(async (src) => {
        return fetch(`//${window.location.host}${routebase}${src}`).then(async (res) => {
          ...
          if (!longVer)
            throw new Error(`couldn't map DAWG version to a DAWG kind: ${ver} = ${longVer}`)
    ...
    .catch((err: Error) => {
      ...
      router.push({
        path: '/error',
        query: { err: ErrorCode.Application, detail: serializeErrorDetails(err) }
      })
    })
    .then(() => {
