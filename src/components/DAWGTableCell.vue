...
const shownIam = ref<Set<string>>(new Set())
const toggleIam = (key: string) => {
  if (shownIam.value.has(key)) shownIam.value.delete(key)
  else shownIam.value.add(key)
}
...
const copyTerraform = async (id: string, key: string) => {
  const ref = await getLatestRef()
  copyVariant(terraformSnippet(id, ref), key)
}
</script>
      ...
      <template v-for="(list, key) in (props.contents as MapOfLists)" :key="key">
        <template v-if="key != 'default'">
          <dt class="monospace" v-bind:id="(key as string).split('/')[1]">
            ...
            <div v-if="shownIam.has(key as string) && props.iamMembers?.[key as string]?.length" class="iam-panel">
              <div class="iam-panel-header">
                <span class="meta-label">GCP IAM members<span class="help-wrapper"><span class="help-icon">?</span><span class="help-tooltip">Verbatim members of this subgroup's IAM binding, as exported from terraform state.</span></span>:</span>
                <span class="copy-btn" @click="copyVariant((props.iamMembers[key as string] ?? []).join('\n'), key + ':iam')">📋<span class="copy-tooltip">{{ copiedVariant === key + ':iam' ? 'copied' : 'copy IAM members' }}</span></span>
              </div>
              <ul class="iam-list">
                <li v-for="m in props.iamMembers[key as string]" :key="m">{{ m }}</li>
              </ul>
            </div>
          </dt>
          <dd>
...
.iam-panel {
  ...
  background: #f9fafb;
  border-left: 3px solid var(--dawg-blue);
  border-radius: 0.25rem;
  font-size: 0.85rem;
}
