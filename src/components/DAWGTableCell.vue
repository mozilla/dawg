...
<template>
  <td class="px-2 py-1">
    <template v-if="!props || !props.contents || props.contents == undefined"> (no data) </template>
    ...
    <template v-if="display === DisplayMode.ListOfLinks">
      <IconLink
        v-for="(link, i) in props.contents as ListOfText"
        :key="i"
        :href="link"
        :auto-text="false"
      />
    ...
    <ul
      v-else-if="
        display === DisplayMode.ListOfText &&
        props.contents &&
        (props.contents as string[]).length > 1
      "
    >
