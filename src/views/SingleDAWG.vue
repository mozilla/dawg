...
onBeforeMount(() => {
  if (!datamap)
    return router.push({
      path: '/error',
      query: {
        err: ErrorCode.Application,
        detail: serializeErrorDetails(`datamap was not loaded got ${datamap}`)
      }
    })
  ...
  if (!datamap.value.has(id.value)) {
    ...
    router.push({
      path: '/error',
      query: { err: ErrorCode.NotFound404, dawgid: encodeURIComponent(id.value) }
    })
  }
  ...
  <div class="container">
    ...
    <template v-for="ver in shortVersions">
      <div v-if="dawghouse?.has(ver)" :key="ver">
        ...
        <nav>
          <IconLink
            v-for="(link, key) in dawghouse.get(ver)?.links as ListOfLinks"
            v-bind:key
            :href="link"
            :autoText="true"
          />
