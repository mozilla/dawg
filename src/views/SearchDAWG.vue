...
const filterFunc = computed(() => {
  return isRegex.value
    ? (c: string): boolean =>
        searchregexp.value instanceof Error == false && searchregexp.value.test(c)
    : (c: string): boolean =>
        !!searchstring.value && c?.toLowerCase().includes(searchstring.value.toLowerCase())
})
...
<template>
  <header class="text-center">
    <p class="text-lg font-normal text-gray-500 lg:text-xl dark:text-gray-400">
      Effortlessly search and explore data access workgroups - Compiled from aggregated Terraform
      State data.
    </p>
  </header>
  <div class="search-wrapper">
    ...
    <div
      ...
      role="alert"
      v-if="isRegex && searchregexp instanceof Error == true"
    >
